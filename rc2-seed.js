(()=>{
  ...
  const PACKS={
    ...
    b4:{url:'./data/seed-r002-b4.json',label:'Lô R-002 B4 theo chặng'}
  };
  ...
  window.importHaTienSeed=()=>importSeed('hatien');window.importR002CorridorSeed=()=>importSeed('corridor');window.importR002B2Seed=()=>importSeed('b2');window.importR002B3Seed=()=>importSeed('b3');window.importR002B4Seed=()=>importSeed('b4');
  const priorRender=window.render;window.render=function renderWithSeed(){priorRender();if(view!=='library')return;const heading=[...document.querySelectorAll('#main h2')].find(x=>x.textContent.includes('Thư viện'));const actions=heading?.parentElement?.querySelector('.actions');if(!actions)return;const buttons=[['seedHaTien','Nạp lô Hà Tiên',importHaTienSeed],['seedR002Corridor','Nạp hành lang R-002',importR002CorridorSeed],['seedR002B2','Nạp R-002 B2 ăn/xăng',importR002B2Seed],['seedR002B3','Nạp R-002 B3 ăn/dịch vụ',importR002B3Seed],['seedR002B4','Nạp R-002 B4 theo chặng',importR002B4Seed]];for(const[key,label,handler]of buttons){const attr=`data-${key.replace(/[A-Z]/g,m=>'-'+m.toLowerCase())}`;if(actions.querySelector(`[${attr}]`))continue;const b=document.createElement('button');b.dataset[key]='1';b.textContent=label;b.onclick=handler;actions.appendChild(b)}};
})();
